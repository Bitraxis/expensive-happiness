...
class NextGenerator(inkex.base.TempDirMixin, inkex.base.InkscapeExtension):
    ...
    def add_arguments(self, pars):
        ...
        pars.add_argument("-t", "--tab", type=str, default="", help="not needed at all")
        pars.add_argument("-l", "--helptabs", type=str, default="", help="not needed at all")
            ...
            if self.options.num_sets == 1:
                for row in data:
                    ...
                    for i, (key, value) in enumerate(row.items()):
                        ...
                        # in case of there not being a valid variable name and we reached the last possible replacement
                ...
                for export_file_num in range(num_exports):
                    ...
                    self.new_doc = self.document
                    self.new_doc = self.remove_absref(self.new_doc)
    ...
    def remove_absref(self, doc):
        """Reduces likelihood of image names occurring twice as often than they actually appear in the document"""
        doc = re.sub('sodipodi:absref=".*"', '', doc)
        return doc
